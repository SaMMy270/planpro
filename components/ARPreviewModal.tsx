...
const ARPreviewModal: React.FC<ARPreviewModalProps> = ({ product, onClose }) => {
  const [viewMode, setViewMode] = useState<'qr' | 'live'>('qr');
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 md:p-12">
      ...
      <div className="relative w-full h-full max-w-6xl max-h-[850px] bg-white rounded-[40px] md:rounded-[60px] overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in zoom-in-95 duration-500">
          ...
          {viewMode === 'qr' ? (
            <div className="w-full h-full relative flex items-center justify-center p-12">
              <div className="absolute inset-0 opacity-40 pointer-events-none">
                 <div className="w-full h-full bg-[linear-gradient(to_right,#0000000a_1px,transparent_1px),linear-gradient(to_bottom,#0000000a_1px,transparent_1px)] bg-[size:30px_30px]"></div>
              </div>
              <div className="relative z-10 space-y-8 flex flex-col items-center text-center">
                <div className="w-24 h-24 bg-white rounded-[40px] flex items-center justify-center border border-black/5 animate-subtle-bounce shadow-xl">
                  <Smartphone size={40} className="text-black/20" strokeWidth={1} />
                </div>
                <div className="space-y-3">
                  <h4 className="text-3xl font-serif text-black tracking-tight">Teleport Experience</h4>
                  <p className="text-black/30 text-xs max-w-xs leading-relaxed uppercase tracking-[0.2em] font-bold">
                    Sync your mobile device for pixel-perfect spatial accuracy.
                  </p>
                </div>
              </div>
              {/* Scanline decoration */}
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-black/5 to-transparent animate-[scan_4s_linear_infinite]" />
            </div>
          ) : (
            /* Live Camera View */
                  ...
                  {placedItems.map((item) => (
                    ...
                    >
                      <div className="relative">
                        ...
                        <img src={item.product.image} className="w-48 md:w-64 h-auto drop-shadow-2xl pointer-events-none" alt={item.product.name} draggable={false} />
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="w-full md:w-[400px] flex flex-col bg-white z-20 border-l border-black/5">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar px-10 pb-10 space-y-12">
            {viewMode === 'qr' ? (
              <div className="space-y-12 animate-in fade-in slide-in-from-right-4 duration-700">
                ...
                {/* Bridge Connection Section - Optimized Size */}
                <div className="space-y-4">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-black/30">Bridge Connection</h4>
                  <div className="p-10 bg-[#F5F5F3] rounded-[48px] border border-black/5 flex flex-col items-center shadow-inner">
                    <div className="bg-white p-6 rounded-[36px] shadow-[0_20px_40px_rgba(0,0,0,0.06)] border border-black/5 group hover:scale-[1.02] transition-transform duration-500">
                       <QrCode size={180} strokeWidth={1.2} className="text-black" />
                    </div>
                    <div className="mt-8 flex items-center gap-3 py-2 px-4 bg-white/50 backdrop-blur-sm rounded-full border border-black/5">
                       <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                       <span className="text-[10px] font-black text-black/40 uppercase tracking-[0.2em]">Ready to Scan</span>
                    </div>
                  </div>
                </div>
